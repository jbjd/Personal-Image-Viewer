...
static PyMemberDef Config_members[] = {
    ...
    {"kb_copy_to_clipboard_as_base64",
     Py_T_OBJECT_EX,
     offsetof(Config, kb_copy_to_clipboard_as_base64),
     Py_READONLY,
     0},
    ...
    {"kb_undo_most_recent_action",
     Py_T_OBJECT_EX,
     offsetof(Config, kb_undo_most_recent_action),
     Py_READONLY,
     0},
...
static inline void Config_SetDefaults(PyObject *self, Config *config) {
    ...
    if (config->kb_copy_to_clipboard_as_base64 == NULL) {
        config->kb_copy_to_clipboard_as_base64 =
            PyObject_GetAttrString(self, VARIABLE_NAME(DEFAULT_KB_COPY_TO_CLIPBOARD_AS_BASE64));
    }
    if (config->kb_move_to_new_file == NULL) {
        config->kb_move_to_new_file =
            PyObject_GetAttrString(self, VARIABLE_NAME(DEFAULT_KB_MOVE_TO_NEW_FILE));
    }
    if (config->kb_optimize_image == NULL) {
        config->kb_optimize_image =
            PyObject_GetAttrString(self, VARIABLE_NAME(DEFAULT_KB_OPTIMIZE_IMAGE));
    }
    ...
    if (config->kb_reload_image == NULL) {
        config->kb_reload_image =
            PyObject_GetAttrString(self, VARIABLE_NAME(DEFAULT_KB_RELOAD_IMAGE));
    }
    ...
    if (config->kb_show_details == NULL) {
        config->kb_show_details =
            PyObject_GetAttrString(self, VARIABLE_NAME(DEFAULT_KB_SHOW_DETAILS));
    }
    if (config->kb_undo_most_recent_action == NULL) {
        config->kb_undo_most_recent_action =
            PyObject_GetAttrString(self, VARIABLE_NAME(DEFAULT_KB_UNDO_MOST_RECENT_ACTION));
    }
    if (config->ui_background_color == NULL) {
        config->ui_background_color =
            PyObject_GetAttrString(self, VARIABLE_NAME(DEFAULT_UI_BACKGROUND_COLOR));
    }
...
static int config_exec(PyObject *module) {
    if (unlikely(
            ...
            PyModule_AddIntConstant(
                module, VARIABLE_NAME(DEFAULT_CACHE_SIZE), DEFAULT_CACHE_SIZE) ||
            PyModule_AddStringConstant(module,
                                       VARIABLE_NAME(DEFAULT_KB_COPY_TO_CLIPBOARD_AS_BASE64),
                                       DEFAULT_KB_COPY_TO_CLIPBOARD_AS_BASE64) ||
            PyModule_AddStringConstant(
                module, VARIABLE_NAME(DEFAULT_KB_MOVE_TO_NEW_FILE), DEFAULT_KB_MOVE_TO_NEW_FILE) ||
            PyModule_AddStringConstant(
                module, VARIABLE_NAME(DEFAULT_KB_OPTIMIZE_IMAGE), DEFAULT_KB_OPTIMIZE_IMAGE) ||
            PyModule_AddStringConstant(
                module, VARIABLE_NAME(DEFAULT_KB_REFRESH), DEFAULT_KB_REFRESH) ||
            PyModule_AddStringConstant(
                module, VARIABLE_NAME(DEFAULT_KB_RELOAD_IMAGE), DEFAULT_KB_RELOAD_IMAGE) ||
            PyModule_AddStringConstant(
                module, VARIABLE_NAME(DEFAULT_KB_RENAME), DEFAULT_KB_RENAME) ||
            PyModule_AddStringConstant(
                module, VARIABLE_NAME(DEFAULT_KB_SHOW_DETAILS), DEFAULT_KB_SHOW_DETAILS) ||
            PyModule_AddStringConstant(module,
                                       VARIABLE_NAME(DEFAULT_KB_UNDO_MOST_RECENT_ACTION),
                                       DEFAULT_KB_UNDO_MOST_RECENT_ACTION) ||
            PyModule_AddStringConstant(
                module, VARIABLE_NAME(DEFAULT_UI_BACKGROUND_COLOR), DEFAULT_UI_BACKGROUND_COLOR) ||
