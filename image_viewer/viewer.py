...
class ViewerApp:
    ...
    def _load_assets(
        ...
        minify_button.create(ButtonName.MINIFY, button_x_offset)
        ...
        rename_button.create(ButtonName.RENAME)
    ...
    def copy_to_clipboard_as_base64(self, _: Event) -> None:
        ...
        if os.name == "nt":
            ...
            image_base64: str = binascii.b2a_base64(
                self.image_io.image_view.view, newline=False
    ...
    def handle_map(self, event: Event) -> None:
        """Explicitly handle regaining focus such as when clicked from taskbar
        on Windows. Tkinter does not handle this properly and caused two
        FocusIn events which broke other logic in the program.

        :param event: An Event generated by tkinter."""
    ...
    def redraw(self, event: Event) -> None:
        """Reloads the current image if its determined that its cache is out of date.

        :param event: An Event generated by tkinter."""

        if (
            event.widget is not self.app
            or self.file_manager.current_image_cache_still_fresh()
        ):
            return
