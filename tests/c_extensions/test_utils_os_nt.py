...
from tests.conftest import EXAMPLE_GIF_PATH, ONLY_ON_WINDOWS
...
def test_read_buffer_as_base64_and_copy_to_clipboard() -> None:
    image_buffer = read_image_into_buffer(EXAMPLE_GIF_PATH)
    assert image_buffer is not None, "Failed to setup image buffer"

    read_buffer_as_base64_and_copy_to_clipboard(image_buffer)

    assert (
        read_clipboard() == "R0lGODdhAgACAKEEAKykvbi1vNLQxd3c2CwAAAAAAgACAAACA0QmBQA7"
    )
